...
typedef struct field_type_t {
  ...
  __attribute__((annotate("scalar(range(   0,   3) final)"))) double *energy0;      // 2D array
  ...
  __attribute__((annotate("scalar(range(  -1,  11) final)"))) double *vertexx;   // 1D array
  __attribute__((annotate("scalar(range(  -1,  11) final)"))) double *vertexy;   // 1D array
