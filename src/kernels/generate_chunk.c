...
void kernel_generate_chunk(int *xmin, int *xmax, int *ymin, int *ymax, double *vertexx, double *vertexy,
                              double *cellx, double *celly, double *density0, double *energy0, double *xvel0,
                              double *yvel0, int *nmbr_f_stts, double *state_density, double *state_energy,
...
#pragma ivdep
      for (j = x_min - 2; j <= x_max + 2; j++) {
        if (state_geometry[FTNREF1D(state, 1)] == g_rect) {
          ...
          radius = sqrt((cellx[FTNREF1D(j, x_min - 2)] - x_cent) * (cellx[FTNREF1D(j, x_min - 2)] - x_cent) +
